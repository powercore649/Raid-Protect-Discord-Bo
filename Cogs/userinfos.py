...
# ------------------------------------------------------ #  
    ...
    async def userinfos (self, ctx, member):
        ...
        if member is not None:
            embed = discord.Embed(title=self.bot.translate.msg(ctx.guild.id, "userinfos", "USER_INFORMATIONS").format(member.name), description="[**Serveur Discord**](https://discord.gg/MS6TMgRfqB)", color=randint(0, 0xffffff))
